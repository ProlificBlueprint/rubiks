...
const moveAvailableSqByDirection = (cubes, direction) => {
  let movableCube = null;
  ...
  cubes.map(cube => {
    ...
    for (var i = 0; i < intersects.length; i++) {
      if (!intersects[i]) {
        ...
        switch (direction) {
          case "T":
            gsap.to(cube.position, { duration: _duration, delay: _delay, y: cube.position.y + 1, ease: _easing, onComplete: ()=> {
              cube.userData.checkCombinationMatches();
            } })
            break;

          case "R":
            gsap.to(cube.position, { duration: _duration, delay: _delay, x: cube.position.x + 1, ease: _easing, onComplete: ()=> {
              cube.userData.checkCombinationMatches();
            } })
            break;
...
export { moveAvailableSqByDirection };